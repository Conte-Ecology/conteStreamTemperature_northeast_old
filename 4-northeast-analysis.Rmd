...
formulae <- list(fixed.form = fixed.form, site.form = site.form, year.form = year.form)
...
system.time(M.ar1 <- modelRegionalTempAR1(tempDataSyncS, formulae = formulae, firstObsRows = firstObsRows, evalRows = evalRows, n.burn = 1, n.it = 100, n.thin = 1, nc = 3, coda = coda.tf, param.list = monitor.params)) # Slow with AR1: ~3-6 min per 100 iterations (13 min per 100 iter for site AR)
...
B.fixed$coef <- c("(Intercept)", attr(terms.formula(formulae$fixed.form), "term.labels"))

B.ar1 <- nameCoefs(coef.summary = coef.summary, rand.levels = levels(as.factor(tempDataSyncS$site)), family = 'B.ar1')
names(B.ar1) <- c("Parameter", "mean", "sd", "qLo", "qHi", "index", "site")
#B.ar1 <- dplyr::filter(coef.summary, grepl('^B.ar1', coef.summary$Parameter))

B.year <- nameCoefs(coef.summary = coef.summary, rand.levels = levels(as.factor(tempDataSyncS$year)), family = "B.year", form = formulae$year.form, name = "year")

B.site <- nameCoefs(coef.summary = coef.summary, rand.levels = levels(as.factor(tempDataSyncS$site)), family = "B.site", form = formulae$site.form, name = "site")

B.huc <- nameCoefs(coef.summary = coef.summary, rand.levels = levels(as.factor(tempDataSyncS$HUC8)), family = "B.huc", form = formulae$site.form, name = "huc")

mu.huc <- nameCoefs(coef.summary = coef.summary, rand.levels = levels(as.factor(tempDataSyncS$HUC8)), family = "mu.huc", name = "huc")
...
tempDataSyncS$Pred <- predictTemp(data = tempDataSyncS, B.fixed = B.fixed, B.site = B.site, B.huc = B.huc, B.year = B.year, B.ar1 = B.ar1)

tempDataSyncValidS$Pred <- NA
tempDataSyncValidS$Pred <- predictTemp(data = tempDataSyncValidS, firstObsRows = firstObsRows.valid, evalRows = evalRows.valid, B.fixed = B.fixed, B.site = B.site, B.huc = B.huc, B.year = B.year, B.ar1 = B.ar1)


library(ggplot2)
ggplot(tempDataSyncValidS, aes(temp, Pred)) + geom_point() + geom_abline(aes(1,1), colour = 'blue')
tempDataSyncValidS$resid.r <- tempDataSyncValidS$temp - tempDataSyncValidS$Pred
...
for(i in 1:length(firstObsRows$rowNum)) {
  trend[firstObsRows$rowNum[i]] <- as.numeric(
    B.fixed$mean %*% t(data.fixed[firstObsRows$rowNum[i], ]) + 
      dplyr::filter(B.huc, huc == as.character(tempDataSyncS$HUC8[firstObsRows$rowNum[i]]))$mean %*% t(data.random.sites[firstObsRows$rowNum[i], ]) + 
      dplyr::filter(B.site, site == as.character(tempDataSyncS$site[firstObsRows$rowNum[i]]))$mean %*% t(data.random.sites[firstObsRows$rowNum[i], ]) + 
      dplyr::filter(B.year, year == as.character(tempDataSyncS$year[firstObsRows$rowNum[i]]))$mean %*% t(data.random.years[firstObsRows$rowNum[i], ])
    )
...
for(i in 1:length(evalRows$rowNum)) {
  trend[evalRows$rowNum[i]] <- as.numeric(
    B.fixed$mean %*% t(data.fixed[evalRows$rowNum[i], ]) + 
      dplyr::filter(B.huc, huc == as.character(tempDataSyncS$HUC8[evalRows$rowNum[i]]))$mean %*% t(data.random.sites[evalRows$rowNum[i], ]) + 
      dplyr::filter(B.site, site == as.character(tempDataSyncS$site[evalRows$rowNum[i]]))$mean %*% t(data.random.sites[evalRows$rowNum[i], ]) + 
      dplyr::filter(B.year, year == as.character(tempDataSyncS$year[evalRows$rowNum[i]]))$mean %*% t(data.random.years[evalRows$rowNum[i], ]) 
    )
  
  tempDataSyncS$Pred[evalRows$rowNum[i]] <- trend[evalRows$rowNum[i]] + 
    dplyr::filter(B.ar1, site == as.character(tempDataSyncS$site[evalRows$rowNum[i]]))$mean * (tempDataSyncS$temp[evalRows$rowNum[i]-1] - trend[evalRows$rowNum[i]-1])
  #as.numeric(B.ar1$mean * (tempDataSyncS$temp[evalRows$rowNum[i]-1] - trend[evalRows$rowNum[i]-1]))
  }
