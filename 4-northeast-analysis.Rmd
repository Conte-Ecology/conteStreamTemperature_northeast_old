...
data.random.sites <- data.frame(intercept.site = 1, 
                     ...
                     airT.drainage = tempDataSyncS$airTemp * tempDataSyncS$TotDASqKM, # effect of interaction with random and fixed effects?
                     airT.precip = tempDataSyncS$airTemp * tempDataSyncS$prcp,
                     airT.precip2 = tempDataSyncS$airTemp * tempDataSyncS$prcpLagged1) # , swe = tempDataSyncS$swe
...
coda.tf <- T # currently only works in full for TRUE (using coda.samples)
system.time(M.ar1 <- modelRegionalTempAR1(tempDataSyncS, data.fixed=data.fixed, data.random.sites=data.random.sites, data.random.years=data.random.years, deployments = deploy.vect, n.burn = 3000, n.it = 1000, n.thin = 1, nc = 3, coda = coda.tf, param.list = monitor.params)) # Slow with AR1: ~3-6 min per 100 iterations 
...
ggs_traceplot(ggs.ar1, family = "B.ar1")
ggs_traceplot(ggs.ar1, family = "sigma.b.site") # no mixing of precipLag2 variance
