...
tbl_df(tempDataSync)
tempDataSync <- arrange(tempDataSync, HUC8, site, date)

# Within each site, if column above doesn't equal date + 1 then start new deployment ID
# SLOW - consider rewriting or moving to C++
tempDataSync$deployID <- NA
tempDataSync$deployID[1] <- 1
for(i in 2:nrow(tempDataSync)) {
  if(tempDataSync$site[i] == tempDataSync$site[i-1] & 
       tempDataSync$date[i] == tempDataSync$date[i -1] + 1) {
    tempDataSync$deployID[i] <- tempDataSync$deployID[i-1]
    } else {
      tempDataSync$deployID[i] <- tempDataSync$deployID[i-1] + 1
      }
  }

head(tempDataSync, 100)
summary(tempDataSync)
...
```{r make AR vector}
deploy.length <- table(tempDataSync$deployID) # named vector, length of each deployment (never longer than the syncronized season)
...
save(tempDataSync, tempDataSyncS, tempDataSyncValid, tempDataSyncValidS, deploy.vect, file=paste0(dataOutDir, 'tempDataSync.RData'))
