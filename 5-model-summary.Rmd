...
#B.fixed <- extractCoef(means.mat = ar1.mat2, data = data.fixed, family.name = "B.0")
...
#B.year.ggs <- ggs(M.ar1, family = "B.year")
#str(B.year.ggs)
...
#foo <- avgCoefs.ggs(B.year.ggs)#[1:64, ]
...
#B.year.mean <- filter(coef.summary, grepl('^B.year', coef.summary$Parameter))

#B.year.mean$index <- as.numeric(sub(".*?([0-9]+),([0-9]).", replacement = "\\1", B.year.mean$Parameter))
#B.year.mean$index2 <- as.numeric(sub(".*?([0-9]+),([0-9]).", replacement = "\\2", B.year.mean$Parameter))

#df.year <- data.frame(year = levels(as.factor(tempDataSyncS$year)), index = 1:length(unique(tempDataSyncS$year)))
#df.coef <- data.frame(coef = colnames(data.random.years), index2 = 1:length(colnames(data.random.years)))

# recombine to link year and index number of the year
#B.year.mean <- dplyr::left_join(B.year.mean, df.year, by = "index")
#B.year.mean <- dplyr::left_join(B.year.mean, df.coef, by = "index2")
...
# Alternative: indexing based on the order found in the df but index sorted by character
# B.year.mean <- data.frame(B.year.mean, year = rep(unique(as.factor(tempDataSyncS$year)), each = 4))

# Alternative: indexing based on numeric sorted year
#B.year.mean$index <- as.numeric(sub(".*?([0-9]+),[0-9].", replacement = "\\1", B.year.mean$Parameter))
#B.year.mean <- arrange(B.year.mean, index)
#B.year.mean <- data.frame(B.year.mean, year = rep(min(tempDataSyncS$year):max(tempDataSyncS$year), each = 4)) # WORKS!!!!!!!!!!!!!!!
...
extractCoef <- function (means.mat, data = NULL, family.name, groupIDs = NULL, rand.spec = FALSE) {
  require(dplyr)
